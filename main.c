...
/**
  ...
  * All rights reserved.
  * This software is licensed under terms at can be found in the LICENSE file
...
typedef struct {
    ...
    uint32_t* counterOutside;
    uint32_t* counterInside;
    GPIO_PinState* state;
} HallSensor;

//uint32_t previousMillisA = 0;
//uint32_t previousMillisB = 0;
//uint32_t previousMillisC = 0;
uint32_t currentMillis = 0;

//VARIABLES DE TESTEO
uint32_t counterOutsideA = 0;
uint32_t counterInsideA = 0;
uint32_t counterOutsideB = 0;
uint32_t counterInsideB = 0;
uint32_t counterOutsideC = 0;
uint32_t counterInsideC = 0;
volatile int casetest;
...
void switchfase(){
	if (HALLA_State == 1 && HALLB_State == 0 && HALLC_State == 1) {
		...
		nuevaFase = 1;} /*C-B*/
//	if (DMS_State == 1){ HIGH_SIGNAL = 320; while (HIGH_SIGNAL > 280){ HIGH_SIGNAL--;	}}
//	if (DMS_State == 0){ HIGH_SIGNAL = 0; /* nuevaFase = 0; */}
...
void SetMotorPhase(){
	switch(nuevaFase){
		case 0:
			...
			/* SDA OFF */ TIM2 -> CCR4 = HIGH_SIGNAL;
			/* SDB OFF */ TIM3 -> CCR2 = 0;
			/* SDC OFF */ TIM4 -> CCR2 = 0;
			/* INA OFF */ TIM2 -> CCR1 = HIGH_SIGNAL;
			/* INB OFF */ TIM3 -> CCR1 = 0;
			/* INC OFF */ TIM4 -> CCR1 = 0;
			break;
		/* Phase1 C-B */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	...
	HallSensor sensors[] = {
			{HALL_A_Pin, 0, &counterOutsideA, &counterInsideA, &HALLA_State},
	        {HALL_B_Pin, 0, &counterOutsideB, &counterInsideB, &HALLB_State},
	        {HALL_C_Pin, 0, &counterOutsideC, &counterInsideC, &HALLC_State}
	};

	uint32_t currentMillis = HAL_GetTick();

	for (int i = 0; i < sizeof(sensors) / sizeof(HallSensor); i++) {
	    if (GPIO_Pin == sensors[i].pin) {
	          (*(sensors[i].counterOutside))++; // For testing only
	          if (currentMillis - sensors[i].previousMillis > 10) {
	              (*(sensors[i].counterInside))++; // For testing only
	               *(sensors[i].state) = HAL_GPIO_ReadPin(GPIOA, sensors[i].pin);
