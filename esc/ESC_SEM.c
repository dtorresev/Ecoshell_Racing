...
while (1){
	...
	HAL_ADC_Stop(&hadc2);
	//Sample_signal = 0.8791208791 * ThrottleSignal;} //failed
	...
	if (DMS_State == 1){
		HIGH_SIGNAL = 3000;
		if (ThrottleSignal > 4000){Sample_signal = 3600;}
		else if (ThrottleSignal<2000){Sample_signal = 0;}
		else {Sample_signal = 0.4295942721*ThrottleSignal + 1760;}
		if (Sample_signal >= HIGH_SIGNAL){HIGH_SIGNAL = Sample_signal;}
	}
	else {HIGH_SIGNAL = 0;}
	SetMotorPhase(nuevaFase);
    /* USER CODE END WHILE */
